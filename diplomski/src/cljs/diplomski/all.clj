...
(defn gubici-za-odredjeni-datum [datum] (if (nil? datum)
                                          "aa"
                                          (d/q `[:find ?c ?t ?p
                                               :where
                                               [?c :gubici/broj_MM ?t ~(first (id-transakcije-za-odr-datum datum))]
                                               [?c :gubici/proc_kompletnosti ?p]]
                                             (d/db conn))))
...
;
;(defn red [kol1 kol2 kol3] [:tr [:td kol1] [:td kol2] [:td kol3]])
;
; (defn svi-redovi [datum] (map #(red (% 0) (% 1) (% 2)) (gubici-za-odredjeni-datum datum))) 
; 
; (defn generate-table[datum]
...
;
; 
;(defn comobo-opcija [vrijednost] [:option {:value vrijednost}  vrijednost])
;
(defn sve-opcije [] (map #(comobo-opcija (format "%1$tF" (first %))) (sve-trans-gubici)))
...
; 
; (defn gtable
;   ([] "<table id = \"as123\"></table>")
;   ([datum1234] (if (nil? datum1234)
;                  "aaa"
;                   (html (generate-table datum1234)))))
; 
; ;
;
